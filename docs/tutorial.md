...
├── knowledge-pages.json              ← wiki page-to-tier mapping (edit this)
...
@~/.claude/team_knowledge.md             ← synced from your wiki (Confluence)
```
...
If your team has a wiki (Confluence, Notion, GitBook), connect it so knowledge stays in sync. The pull script ships with a **Confluence adapter** — we'll use that here.

### Step 1: Add Confluence credentials

```bash
mkdir -p ~/.config/confluence
cat > ~/.config/confluence/.env << 'EOF'
CONFLUENCE_BASE_URL=https://your-org.atlassian.net
CONFLUENCE_USERNAME=jane.smith@mycompany.com
CONFLUENCE_API_TOKEN=your-api-token-here
EOF
```

### Step 2: Map your Confluence pages to the 4 tiers

Edit `~/.claude/knowledge-pages.json`:
...
You should see your Confluence content pulled and assembled into a local file.

## Minute 20-25: Add a Dependency Map (Tier 2)

Create a Confluence page with your cross-service topology. This becomes Tier 2. When you run `pull_knowledge.sh`, it pulls this page and assembles it into `~/.claude/team_knowledge.md` automatically. (Don't edit `team_knowledge.md` directly — it's a cache file that gets overwritten on each pull.)

Example dependency map content for your Confluence page:
...
Claude: Use mycompany-dev.team_dataset_dev. Never use mycompany-infra-dev —
...
### Test 3: Knowledge push-back
```
You: I just discovered that CLI queries ignore SET @@query_label —
     you need --label flags instead.

Claude: LEARNING DETECTED — review before push
        Title:    CLI Query Labels Need --label Flags
        Category: standard
        Target:   Team Standards on Confluence
        ...
        Push to Confluence? [approve / edit / skip]
```

If Claude suggests pushing the learning to Confluence, the bidirectional flow is working.
...
| Roll out to teammates | 30 min/person | [CONTRIBUTING.md](../CONTRIBUTING.md) |

## Troubleshooting

### "team_knowledge.md is empty"
- Check Confluence credentials: `cat ~/.config/confluence/.env`
- Check page IDs: `cat ~/.claude/knowledge-pages.json`
