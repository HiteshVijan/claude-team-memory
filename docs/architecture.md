...
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐    │
│  │ Tier 3: Module Detail (loaded per-repo, ~100 lines)      │    │
...
### Tier 3: Module Detail (~100 lines, loaded per-repo)
...
**Loading strategy:**
1. The detail for your current repo is included in `team_knowledge.md` automatically
2. Details for other repos are pre-cached at `~/.claude/cache/wiki/{repo-name}.md`
3. Claude reads the cached file when a cross-service question requires it
4. Wiki API is only called if the cache file is missing

**Supported wikis for caching:**
- **Confluence** — REST API v1 with body.storage expansion
- **Notion** — API with block children retrieval
- **GitBook** — API or git-based markdown sync
- **Git wiki** — direct git clone of a wiki repo (cheapest option)
...
UserPromptSubmit Hook (Claude Code built-in)
    ...
    │         (shared standards + dependency map + domain pages + module detail)
    │
...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "type": "command",
        "command": "bash ~/.claude/scripts/pull_knowledge.sh"
      }
    ]
  }
}
...
**Wiki sync** (30-minute cache TTL):
```
1. Check if ~/.claude/team_knowledge.md exists and is < 30 min old
2. If fresh → skip (instant, no network call)
3. If stale → call wiki API:
   a. Pull shared standards page (Tier 1)
   b. Pull dependency map page (Tier 2)
   c. Pull domain pages (if configured)
   d. Pull module detail for current repo (Tier 3)
   e. Assemble all pages into one file: ~/.claude/team_knowledge.md
   f. Cache individual module pages in ~/.claude/cache/wiki/ (Tier 4)
```
...
- Team knowledge: shared standards + dependency map + module detail (Tiers 1-3, ~250-350 lines)

**Total: ~400-560 lines loaded before you type anything.**

### Step 4: On-Demand References (Tier 4)
...
Claude processes user's first prompt
  ...
  └── Already has: team standards, dependency map,
      module detail, skill routing — zero questions asked
```
...
Don't dump 2000 lines into context. Load 250 lines (Tiers 1-2) always, 100 lines (Tier 3) per-repo, and the rest on demand. This keeps the context window efficient while ensuring the AI never lacks critical information.

### 2. Wiki as Source of Truth
Local files are caches, not sources. The wiki is authoritative. This means:
...
| Team Standards | ~50 | Always |
| Skill Routing | ~130 | Always |
| Team Knowledge (shared + dependency map) | ~250 | Always |
| Module Detail (current repo) | ~100 | Per-repo |
| **Total baseline** | **~560** | **Every session** |
| On-demand references | ~100-300 | When needed |

Claude Code's context window is large enough that 560 lines of persistent knowledge is a worthwhile tradeoff — it prevents far more wasted context from re-explaining things.
